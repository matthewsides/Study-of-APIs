...
The current iteration facilitates player input, movement, collision and gravity, however future builds in order to meet the design and direction of this project and improve upon the current build will include Main menu camera movement, AI and shooting. Camera movement could be achieved through using a free API that can be found from Unity's asset store known as Bézier Path Creator, which is an intuitive and lightweight editor for quickly creating smooth paths in the editor. Through using Bezier path creator it would be possible to make objects move along these paths thus achieving what was sought out in the conceptual designs and making a better asthetic rather than simply using a static menu background. Whilst the AI could be incorporated through using similiar techniques applied to the player i.e. transform,rigidbodies and colliders. This in addition to Unitys Navmeshagent a component of Unitys scripting API which allows characters to move using the navmesh a class that can be used to do spatial queries, for instance pathfinding and walkability tests, setting the pathfinding cost for specific area types, and tweaking global behavior of pathfinding and avoidance. Through using this API it would be easier to control an object that follows a path and also determine collision and avoidance in particularly through using the NavMeshobstacle component that can be tweaked through the UnityEngine AI API and is apart of the Navmesh classes. Moreover the application could be improved with the addition of shooting and an API which supports and enables Bullet Physics. This could be done through using the similiar components and methods applied to the player, however additional forces could be used on the rigidbody, like Addforce in order to push the bullet and simulate bullet physics. This Addforce function is also apart of Unitys scripting API, more specifically the UnityEngine API and can be accessed and tweaked via code so long as the object has a rigidbody attached.

Following the prospective improvements that could be made through the use of additional API's, the application could be invisioned to be  be more feature heavy, with variation and substance if such improvements were to be implemented with the support or continious use of the suggested API's.
